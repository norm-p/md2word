...
| New images not inserted | Images referenced in the Markdown (e.g., base64 data URIs) are not added to the DOCX. Existing images are preserved. | Both modes |

## Recent fixes

Fixes are listed newest-first with the review that introduced them.

### R27 -- Bold emphasis, stale bullet removal, heading rename

- **Bold emphasis applied from source Markdown.** Option C+bold detects `**bold**`
  markers in the source MD, matches them to corresponding DOCX paragraphs using
  per-line similarity matching (≥0.75), and splits runs to apply `w:b` formatting.
  Uses word-boundary-aware matching to prevent false positives (e.g., "not" inside
  "notice") and position-aware occurrence selection for repeated words.
- **Stale bullets removed.** Option E detects DOCX bullets with no match (< 0.50
  similarity) in the source MD and removes them. Bullets that were deleted from the
  source Markdown no longer persist in the output.
- **Heading text updated for non-Markdown headings.** Post-mapping scan finds
  unmatched DOCX headings that match all-caps body text lines in the source MD
  (≥0.70 similarity with shared distinctive words). Headings that were renamed in
  the source now reflect the updated text.

### R26 -- Pre-comparison pipeline

- **Pre-comparison eliminates unnecessary AI calls.** Before sending any
  sections to the AI, the tool now converts the base DOCX to Markdown and
  compares it against the source Markdown section-by-section. Sections with
  identical or near-identical content are handled entirely by deterministic
  patching, preserving original DOCX formatting perfectly. Only sections with
  major changes are sent to the AI for regeneration.
- **Assumptions section formatting preserved.** Bold sub-category headers
  (e.g., `**Project Planning & Scope**`) within the Assumptions section are no
  longer lost during updates. Previously, the AI would regenerate the section
  and drop these organizational headers.
- **Sub-heading duplication eliminated.** Sections like "Delivery Experience
  Framework" and "HIGH LEVEL IMPLEMENTATION" no longer produce duplicate
  headings in the output, because the sections are not regenerated by the AI.
- **Text corrections reach all document sections.** Deterministic text
  corrections now run on DOCX sections that were previously unreachable
  because they had no matching Markdown heading.
...
- **Table cell markdown unescaping.** Cells containing underscores (e.g.,
  `PROJ_MODULE_ID_01`) are no longer corrupted by overly aggressive markdown
